...
router.get('/live', async (req, res, next) => {
    try {
        const result = await generalizedSofaScoreService.getLiveEvents('football');
...
router.get('/scheduled', async (req, res, next) => {
    try {
        ...
        const result = await generalizedSofaScoreService.getScheduledEvents('football', date, 24);
        res.json(result.data || { events: [] });
    } catch (error) {
        next(error);
    }
...
router.get('/finished', async (req, res, next) => {
    try {
        const { date } = req.query;
        const result = await generalizedSofaScoreService.getEventsByDate('football', date);
