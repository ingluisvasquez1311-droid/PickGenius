...
router.get('/live', async (req, res, next) => {
    try {
        const result = await generalizedSofaScoreService.getLiveEvents('american-football');
...
router.get('/scheduled', async (req, res, next) => {
    try {
        ...
        const result = await generalizedSofaScoreService.getScheduledEvents('american-football', date, 24);
        res.json(result.data || { events: [] });
    } catch (error) {
        next(error);
    }
...
router.get('/finished', async (req, res, next) => {
    try {
        const { date } = req.query;
        const result = await generalizedSofaScoreService.getEventsByDate('american-football', date);
