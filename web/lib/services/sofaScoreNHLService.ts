...
class SofaScoreNHLService {
    private baseUrl: string = 'https://www.sofascore.com/api/v1';
    ...
    async makeRequest<T = any>(endpoint: string, cacheKey: string, ttlSeconds: number = 3600): Promise<ApiResponse<T>> {
        try {
            ...
            console.error(`❌ SofaScore NHL Error (${endpoint}):`, error.message);
...
export const sofaScoreNHLService = new SofaScoreNHLService();
