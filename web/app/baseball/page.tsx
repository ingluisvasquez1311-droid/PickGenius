...
import { Trophy, Activity, Calendar, ChevronRight, ChevronDown, Star, Target, Zap, ArrowLeft, BarChart3, TrendingUp, Disc, Shield, ArrowRight } from 'lucide-react';
...
export default function BaseballHub() {
    ...
    const baseballLiveCount = liveCounts['baseball'] || 0;

    useEffect(() => {
        const fetchBaseball = async () => {
            setLoading(true);
            try {
                const endpoint = activeFilter === 'live' ? '/api/live/baseball' : '/api/scheduled/baseball';
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-8 space-y-10">
                        <div className="flex items-center gap-6">
                            <h2 className="text-4xl font-black italic uppercase tracking-tighter">
                                {activeFilter === 'live' ? 'En el Diamante' : 'Próximos Pitcheos'}
                            </h2>
                            <div className="flex-1 h-[2px] bg-gradient-to-r from-white/20 to-transparent"></div>
                            <div className="flex items-center gap-2 px-4 py-1.5 bg-white/5 rounded-full border border-white/10">
                                <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Pitcher Radar Active</span>
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        ...
                        <div className="relative group">
                            ...
                            <div className="relative bg-[#080808] border-2 border-white/10 rounded-[3rem] p-10 space-y-6">
                                ...
                                <div className="flex items-center gap-4 border-b border-white/5 pb-6 mt-6">
                                    ...
                                    <h3 className="text-2xl font-black italic uppercase tracking-tighter">ZONA DE STRIKE IA</h3>
                                </div>
                                <p className="text-xs text-gray-500 font-bold leading-relaxed uppercase tracking-wider">
                                    Tendencia de <span className="text-blue-500">altas carreras</span> hoy en parques secos. El viento favorece a los bateadores en el noreste.
                                </p>
                                <div className="bg-white/5 p-6 rounded-2xl border border-white/10 space-y-4">
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] uppercase font-black text-gray-500">NRFI Probability</span>
                                        <span className="text-[10px] font-black text-blue-500 italic">45% MODERATE</span>
                                    </div>
                                    <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                                        <div className="h-full bg-blue-500 w-[45%] shadow-glow-sm"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-[#080808] border-2 border-white/10 rounded-[3rem] p-10 space-y-8">
                            <h4 className="text-xs font-black uppercase text-gray-500 tracking-[0.4em] border-b border-white/5 pb-4">DIAMOND CIRCUITS</h4>
                            <div className="grid gap-3">
                                {['MLB', 'LMP', 'LIDOM', 'NPB'].map(comp => (
                                    <div key={comp} className="group flex justify-between items-center p-5 bg-white/5 border border-white/5 hover:border-blue-500/40 hover:bg-blue-500/5 rounded-2xl transition-all duration-300 cursor-pointer">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 group-hover:text-white">{comp}</span>
                                        <ChevronRight className="w-4 h-4 text-gray-700 group-hover:text-blue-500 transition-all" />
                                    </div>
                                ))}
                            </div>
                        </div>
