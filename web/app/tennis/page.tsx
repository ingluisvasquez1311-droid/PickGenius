...
export default function TennisHub() {
    ...
    useEffect(() => {
        const fetchTennis = async () => {
            setLoading(true);
            try {
                const endpoint = activeFilter === 'live' ? '/api/live/tennis' : '/api/scheduled/tennis';
                const res = await fetch(endpoint);
                const data = await res.json();
                const events = data.events || [];
                setMatches(events);

                // Auto-expand first 3 tournaments (usually Grand Slams/Masters)
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white pt-24 pb-20 px-4 md:px-12 max-w-[1600px] mx-auto relative overflow-hidden">
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                <div className="absolute top-[-20%] right-[-10%] w-[80%] h-[80%] bg-[#00FF88]/20 blur-[180px] animate-pulse rounded-full opacity-60"></div>
                <div className="absolute top-[10%] left-[-20%] w-[70%] h-[70%] bg-emerald-400/15 blur-[200px] rounded-full opacity-40"></div>
                <div className="absolute bottom-[-10%] left-[20%] w-[60%] h-[60%] bg-green-500/10 blur-[180px] rounded-full opacity-30"></div>
                ...
                <div className="absolute top-[40%] left-1/2 -translate-x-1/2 w-full h-[2px] bg-gradient-to-r from-transparent via-[#00FF88]/20 to-transparent blur-sm"></div>
            </div>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10 border-b-2 border-white/10 pb-12">
                    <div className="space-y-4">
                        <div className="flex items-center gap-4">
                            <Link href="/" className="group p-4 glass-card rounded-2xl hover:cyber-border transition-all duration-500">
                                <ArrowLeft className="w-6 h-6 text-gray-400 group-hover:text-[#00FF88]" />
                            </Link>
                            <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-[#00FF88]/10 border border-[#00FF88]/20 text-[#00FF88] text-[10px] font-black uppercase tracking-[0.4em]">
                        ...
                        <h1 className="text-7xl md:text-[12rem] font-black italic tracking-tighter uppercase leading-[0.8] flex flex-col">
                            <span className="text-white drop-shadow-[0_0_80px_rgba(0,255,136,0.3)]">HUB</span>
                            <span className="gradient-text" style={{ background: 'linear-gradient(135deg, #00FF88 10%, #10b981 50%, #6ee7b7 90%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundSize: '200% 200%', animation: 'var(--animate-gradient-shift)' }}>TENIS ELITE</span>
                        </h1>
                        <div className="flex items-center gap-4">
                            <div className="h-px w-24 bg-gradient-to-r from-[#00FF88] to-transparent"></div>
                            <p className="text-gray-500 font-black uppercase tracking-[0.5em] text-[10px]">Precision Tracking & Set Analysis</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-6 w-full lg:w-auto">
                        <div className="flex flex-wrap gap-4">
                            ...
                            <div className="flex glass-card p-1.5 rounded-[1.5rem] border border-white/10">
                                ...
                                >
                                    ...
                                    Live {tennisLiveCount > 0 && (
                                        <span className="ml-1.5 px-2 py-0.5 bg-black/20 rounded-md text-[10px] font-mono">{tennisLiveCount}</span>
                                    )}
                                </button>
                                <button
                                    ...
                                    className={clsx(
                                        "px-8 py-3 rounded-xl font-black uppercase tracking-widest text-[11px] flex items-center gap-3 transition-all duration-500",
                                        activeFilter === 'scheduled' ? "bg-[#00FF88] text-black shadow-[0_0_25px_rgba(0,255,136,0.4)]" : "text-gray-600 hover:text-white hover:bg-white/5"
                                    )}
                                >
                            ...
                            <Link href="/props?sport=tennis" className="group flex items-center gap-4 px-8 py-4 glass-card hover:cyber-border rounded-2xl transition-all duration-500">
                                <Zap className="w-5 h-5 text-yellow-500 group-hover:scale-125 group-hover:rotate-12 transition-all animate-pulse" />
                                <span className="text-[11px] font-black uppercase tracking-[0.2em] text-gray-400 group-hover:text-[#00FF88] italic">Player Props</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-8 space-y-10">
                        <div className="flex items-center gap-6">
                            <h2 className="text-4xl font-black italic uppercase tracking-tighter">
                                {activeFilter === 'live' ? 'En la Cancha Central' : 'Orden de Juego'}
                            </h2>
                            <div className="flex-1 h-[2px] bg-gradient-to-r from-white/20 to-transparent"></div>
                            <div className="flex items-center gap-2 px-4 py-1.5 bg-white/5 rounded-full border border-white/10">
                                <div className="w-2 h-2 rounded-full bg-[#00FF88] animate-pulse"></div>
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Set Logic Active</span>
                            </div>
                        </div>
                        ...
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-white/10 rounded-[4rem] bg-white/[0.01]">
                                ...
                                <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">El radar está limpio en este sector</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        ...
                        <div className="relative group">
                            ...
                            <div className="relative bg-[#080808] border-2 border-white/10 rounded-[3rem] p-10 space-y-6">
                                ...
                                <div className="flex items-center gap-4 border-b border-white/5 pb-6 mt-6">
                                    ...
                                    <h3 className="text-2xl font-black italic uppercase tracking-tighter">VISIÓN DE CANCHA</h3>
                                </div>
                                <p className="text-xs text-gray-500 font-bold leading-relaxed uppercase tracking-wider">
                                    Las pistas rápidas están favoreciendo a los servicios fuertes. <span className="text-[#00FF88]">Se proyectan +12 Aces</span> de promedio en ATP.
                                </p>
                                <div className="bg-white/5 p-6 rounded-2xl border border-white/10 space-y-4">
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] uppercase font-black text-gray-500">Tie-Break Probability</span>
                                        <span className="text-[10px] font-black text-[#00FF88] italic">65% HIGH</span>
                                    </div>
                                    <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                                        <div className="h-full bg-[#00FF88] w-[65%] shadow-glow-sm"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-[#080808] border-2 border-white/10 rounded-[3rem] p-10 space-y-8">
                            <h4 className="text-xs font-black uppercase text-gray-500 tracking-[0.4em] border-b border-white/5 pb-4">NODE CIRCUITS</h4>
                            <div className="grid gap-3">
                                {['ATP Tour', 'WTA Tour', 'Challenger', 'ITF'].map(comp => (
                                    <div key={comp} className="group flex justify-between items-center p-5 bg-white/5 border border-white/5 hover:border-[#00FF88]/40 hover:bg-[#00FF88]/5 rounded-2xl transition-all duration-300 cursor-pointer">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 group-hover:text-white">{comp}</span>
                                        <ChevronRight className="w-4 h-4 text-gray-700 group-hover:text-[#00FF88] transition-all" />
                                    </div>
                                ))}
                            </div>
                        </div>
